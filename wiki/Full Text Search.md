...
With `External`, every write to the source table needs a matching write to the FTS table. The framework can wire that up for you. Set `AutoSync = FtsAutoSync.Triggers` on the attribute and `CreateTable()` will create the standard FTS5 sync triggers (insert, update, delete) on the source table.
...
Match is a marker method that lives on `SQLiteFTS5`. It works inside `Where`.

### String form

```csharp
List<ArticleSearch> hits = db.Table<ArticleSearch>()
    .Where(a => SQLiteFTS5.Match(a, "native AND aot"))
    .ToList();
```
...
```csharp
.Where(a => SQLiteFTS5.Match(a, f => f.Term("native") && f.Term("aot")))

.Where(a => SQLiteFTS5.Match(a, f => f.Phrase("native aot")))

.Where(a => SQLiteFTS5.Match(a, f => f.Prefix("nativ")))

.Where(a => SQLiteFTS5.Match(a, f => f.Near(2, "ahead", "time")))
```
...
```csharp
.Where(a => SQLiteFTS5.Match(a.Title, "native"))

.Where(a => SQLiteFTS5.Match(a.Title, f => f.Prefix("nativ")))
```

To mix a column scope with other terms, use `f.Column` inside the builder lambda:

```csharp
.Where(a => SQLiteFTS5.Match(a,
    f => f.Column(a.Title, f.Prefix("aot")) || f.Term("trim")))
```

## Ranking

`SQLiteFTS5.Rank(entity)` returns the BM25 score of the row. Use it inside `OrderBy`. The per-column weights from `[FullTextIndexed(Weight = ...)]` are applied automatically.

```csharp
db.Table<ArticleSearch>()
    .Where(a => SQLiteFTS5.Match(a, "native"))
    .OrderBy(a => SQLiteFTS5.Rank(a))
    .Take(20)
    .ToList();
```
...
var hits = db.Table<ArticleSearch>()
    ...
    {
        a.Id,
        Title = SQLiteFTS5.Highlight(a, a.Title, "<b>", "</b>"),
        Body = SQLiteFTS5.Snippet(a, a.Body, "<b>", "</b>", "...", 32),
    })
    .ToList();
```
...
var hits = (
    ...
    where SQLiteFTS5.Match(s, "native aot")
    orderby SQLiteFTS5.Rank(s)
