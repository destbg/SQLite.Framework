...
public class StringOperationTests
{
    ...
    public void StringPadLeft()
    {
        ...
        Assert.Equal("SELECT b0.\"BookId\" AS \"Id\",\n       b0.\"BookTitle\" AS \"Title\",\n       b0.\"BookAuthorId\" AS \"AuthorId\",\n       b0.\"BookPrice\" AS \"Price\"\nFROM \"Books\" AS b0\nWHERE (SELECT (CASE WHEN LENGTH(v5) >= v7 THEN v5 ELSE (SUBSTR(REPLACE(HEX(ZEROBLOB(v7 - LENGTH(v5))), '00', @p1), 1, v7 - LENGTH(v5)) || v5) END) FROM (SELECT b0.\"BookTitle\" AS v5, @p0 AS v7)) = @p2", command.CommandText.Replace("\r\n", "\n"));
    }
    ...
    public void StringPadRight()
    {
        ...
        Assert.Equal(" ", command.Parameters[1].Value);
        Assert.Equal("Test    ", command.Parameters[2].Value);
        Assert.Equal("SELECT b0.\"BookId\" AS \"Id\",\n       b0.\"BookTitle\" AS \"Title\",\n       b0.\"BookAuthorId\" AS \"AuthorId\",\n       b0.\"BookPrice\" AS \"Price\"\nFROM \"Books\" AS b0\nWHERE (SELECT (CASE WHEN LENGTH(v5) >= v7 THEN v5 ELSE (v5 || (SUBSTR(REPLACE(HEX(ZEROBLOB(v7 - LENGTH(v5))), '00', @p1), 1, v7 - LENGTH(v5)))) END) FROM (SELECT b0.\"BookTitle\" AS v5, @p0 AS v7)) = @p2", command.CommandText.Replace("\r\n", "\n"));
    }
    ...
    public void StringNullCoalescingInSelect()
    {
        ...
        db.Table<BookWithNotes>().Schema.CreateTable();
