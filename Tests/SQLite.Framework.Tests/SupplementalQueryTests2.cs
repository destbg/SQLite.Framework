...
public class SupplementalQueryTests2
{
    [Fact]
    ...
    public void Window_Lag_OneArgAndTwoArgAndThreeArg_AllTranslate()
    {
        ...
        Assert.Equal("SELECT LAG(b0.\"BookPrice\") OVER ( ORDER BY b0.\"BookId\" ASC) AS \"6\"\nFROM \"Books\" AS b0", sql1);
        Assert.Equal("SELECT LAG(b0.\"BookPrice\", @p0) OVER ( ORDER BY b0.\"BookId\" ASC) AS \"7\"\nFROM \"Books\" AS b0", sql2);
        Assert.Equal("SELECT LAG(b0.\"BookPrice\", @p0, @p1) OVER ( ORDER BY b0.\"BookId\" ASC) AS \"8\"\nFROM \"Books\" AS b0", sql3);
    }
