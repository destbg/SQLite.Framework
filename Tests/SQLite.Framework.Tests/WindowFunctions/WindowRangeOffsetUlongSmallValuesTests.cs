...
{
    [Fact]
    public void SumRangeFollowingOffsetOrderedByUlongSmallValues()
    {
        ...
        List<int> expected = rows.OrderBy(r => r.Id)
            .Select(r => rows.Where(o => o.ULongValue >= r.ULongValue && o.ULongValue <= r.ULongValue + 5UL).Sum(o => o.IntValue))
            .ToList();
        Assert.Equal([3, 3, 2], expected);
        ...
        List<int> actual = db.Table<NumericType>()
            .Select(x => new
            {
                ...
                S = SQLiteWindowFunctions.Sum(x.IntValue).Over().OrderBy(x.ULongValue)
                    .Range(SQLiteFrameBoundary.CurrentRow(), SQLiteFrameBoundary.Following(5)).AsValue()
            })
            .OrderBy(x => x.Id)
            .Select(x => x.S)
            .ToList();
