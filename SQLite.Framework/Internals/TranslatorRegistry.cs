...
{
    [UnconditionalSuppressMessage("AOT", "IL2075", Justification = "Open generic type methods are matched by metadata definition for custom translator registration.")]
    public static bool TryGetMethodTranslator(this SQLiteOptions options, MethodInfo method, [NotNullWhen(true)] out SQLiteMemberTranslator? translator)
    {
        ...
        if (method.DeclaringType!.IsConstructedGenericType)
        {
            ...
            {
                if (openMethod.HasSameMetadataDefinitionAs(method)
